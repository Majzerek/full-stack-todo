...
export const getUsersStatistics = async (
  ...
  try {
    ...
    const usersList = (await usersCollection.find().toArray()).map(
      ({ _id, ...rest }) => ({ id: _id, ...rest }),
    );

    const filteredUsers = usersList.reduce<
      Record<
        StatusEnum.ACTIVE | StatusEnum.PENDING | StatusEnum.DECLINED,
        {
          new: number;
