import { Accordion, AccordionContent, AccordionItem, AccordionTrigger, Badge, Button, Card, CardContent, CardDescription, CardFooter, CardTitle, Loader, Wrapper } from "@/components";
        ...
        {userTasks.map((task) => (
            <Card key={task.id} className={task.isDone ? "bg-border opacity-55 p-2 w-70 h-fit" : "p-2 w-70 h-fit"}>
                ...
                <div className="flex gap-2 flex-wrap">{task.hashTag?.map((hash, index) => (
                  <Badge key={hash} className={`bg-chart-${index + 1}`}>{hash}</Badge>
                ))}</div>
